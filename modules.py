...
class Projection(nn.Module):
    def __init__(self, embed_dim, in_dim, activation="identity", dropout_p=0, norm=False):
        super(Projection, self).__init__()
        self.activation = get_activation_function(activation) if activation.lower() != "identity" else nn.Identity()
        self.dropout = nn.Dropout(p=dropout_p)
        self.norm = nn.LayerNorm(embed_dim) if norm else nn.Identity()
        self.linear_layer = nn.Linear(in_dim, embed_dim)
...
class LearnablePositionEncoding(nn.Module):
    def __init__(self, seq_len, embed_dim, dropout_p=0):
        ...
        self.dropout = nn.Dropout(dropout_p)
        self.pos_embeddings = nn.Parameter(torch.randn(1, seq_len, embed_dim))
...
class FFN(nn.Module):
    def __init__(self, ffn_dim, embed_dim, dropout_p=0, activation="relu"):
...
class Decoder(nn.Module):
    def __init__(self, embed_dim, num_heads, ffn_dim, attn=MultiHeadAttention, non_linearity="relu", dropout_p=0):
        super(Decoder, self).__init__()
        self.multi_attn = attn(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout_p)
        self.ffn = FFN(ffn_dim=ffn_dim, embed_dim=embed_dim, dropout_p=dropout_p, activation=non_linearity)
